...
## Capturing population size

```{r capturingpopulationsize, tidy=TRUE}
d %>%
  summarise(r_set = cor(settlement,log_pop_size, use = "pairwise.complete.obs"),
    r_j = cor(jurisdiction,log_pop_size, use = "pairwise.complete.obs"),
    r_jl = cor(jurisdiction_local,log_pop_size, use = "pairwise.complete.obs"),
    r_sub = cor(as.numeric(subsistence),log_pop_size, use = "pairwise.complete.obs"),
    r_com = cor(comm_size,log_pop_size, use = "pairwise.complete.obs"),
    )
      # r_set     r_j      r_jl      r_sub     r_com
# 1 0.4104416 0.58428 0.2133941 -0.4435946 0.5961375
```
...
# multicollinearity?
#samvif(f2b);summary(f2b) #  "the square root of the VIF for the jth covariate indicates by how much the confidence interval for β̂ j is expanded relative to uncorrelated data"-- by about 2x here, the effects survive it 
# samvif(s5b);summary(s5b) # pastoralist effect is probably gone.
# samvif(k1);summary(k1) # yup
# samvif(k1)[,2] %>% sqrt %>% cbind(as.character(samvif(k1)[,1]))
...
#pdf('figs/cousin_paper_text-predfig.pdf', width = 9, height = 7)
...
pdf('figs/cousin_paper_text-predfig.pdf', width = 9, height = 7)
grid.arrange(p1, p2, p3, p4, legend, ncol = 2, heights = c(1, 1, 0.2))
dev.off()
```
